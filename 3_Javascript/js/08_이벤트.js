...
// 아이디가 test1-1인 요소에 onclick 이벤트 리스너의
// 이벤트 핸들러 제거하기
document.querySelector("#test1-2").onclick = function() {
  // 기존 onclick 의 이벤트핸들러(함수를)
  // null 로 덮어씌움 (이벤트 제거)
  test1a.onclick = null;
...
const testc = document.querySelector("#test1-3");

// testc 요소가 클릭되었을 때 배경색 빨간색으로 변경
testc.onclick = function() {
  testc.style.backgroundColor = "red";
}

// a few moments later..

testc.onclick = function() {
  testc.style.color = "white";
}

// -> onclick에 저장된 값(이벤트 핸들러 함수)
// 값이 덮어씌어지면서 이전코드(배경색 red)가
// 사라지는 문제 발생
...
input3.addEventListener("keyup", function(e){
  if(e.key === "Enter"){
    const inputColor = input3.value.trim();
    box3.style.backgroundColor = inputColor;
    input3.value = "";
  }
});
...
box3.addEventListener("click", function(e){
  ...
  // e.target : 이벤트가 발생한 대상 요소 == this
   ...
   alert(`현재 색상: ${e.target.style.backgroundColor}`);
  // alert(`현재 색상: ${this.style.backgroundColor}`);
});
