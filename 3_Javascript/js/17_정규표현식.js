...
document.querySelector("#btn2").addEventListener("click", () => {
    ...
    // [abcd]
    const regExp1 = /[abcd]/; // 문자열 내에 a,b,c,d 하나라도 일치하는 문자가있는지 확인
    console.log( regExp1.test("apple") );   // true
    console.log( regExp1.test("ssbss") );   // true
    console.log( regExp1.test("qwerty") );  // flase

    //^ (캐럿)
    const regExp2 = /^group/; // 문자열의 시작이 group인지 확인
    console.log( regExp2.test("group"));    // true
    console.log( regExp2.test("100group")); // flase

    //$ (달러)
    const regExp3 = /group$/; // 문자열의 끝이 group인지 확인
     console.log( regExp3.test("group"));    
...
// const 변수 저장 방식
/*const inputPno = document.getElementById("inputPno");
const inputPnoResult = document.getElementById("inputPnoResult");

inputPno.addEventListener("input", (e) => {

  if (e.target.value.length == 0) {
    inputPnoResult.textContent = "";
    return;
  }

  const pnoRegExp = /^\d{6}-[1-4]\d{6}$/;

  if (pnoRegExp.test(e.target.value)) {
    inputPnoResult.textContent = "유효한 주민등록번호 형식입니다.";
    inputPnoResult.style.color = "green";
  } else {
    inputPnoResult.textContent = "형식이 올바르지 않습니다.";
    inputPnoResult.style.color = "red";
  }
});*/
