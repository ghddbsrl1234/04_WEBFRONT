...
.addEventListener("click", () => {
    ...
    // (요소.childNode[index])
    // ul 자식 노드 중 3번째 li 태그를 찾아서 배겨색 pink 지정
    ...
    // 3. 첫 번쨰 자식으로 추가 ( 요소.prepend("내용" or 노드) )
    list[11].prepend("오늘", "저녁", "소주마시지롱");

    // 4. 이전/다음 형제로 추가 (요소.before / 요소.after("내용".노드))
    // ul의 전/후에 내용 추가
    ul.before("이전입니다", "ㅎㅎ");
    ul.after("이후입니다");
});
